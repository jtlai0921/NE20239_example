//$Id: PersistentCollectionType.java,v 1.21 2005/01/29 23:01:54 oneovthafew Exp $
...
public abstract class PersistentCollectionType extends AbstractType implements AssociationType {
	...
	public Object copy(Object original, Object target, SessionImplementor session, Object owner, Map copiedAlready)
	throws HibernateException {
		
		if (original==null) return null;
		if ( !Hibernate.isInitialized(original) ) return target;
		
		java.util.Collection originalCopy = new ArrayList( (java.util.Collection) original );
		java.util.Collection result = target==null ? 
				(java.util.Collection) instantiate( session, session.getFactory().getCollectionPersister(role) ) : 
				(java.util.Collection) target;
		result.clear();
		
		Type elemType = getElementType( session.getFactory() );
		Iterator iter = originalCopy.iterator();
		while ( iter.hasNext() ) {
			result.add( elemType.copy( iter.next(), null, session, owner, copiedAlready ) );
		}
		return result;
	}
